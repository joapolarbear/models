...
def _collect_cpu_info(run_info):
  ...
  try:
    ...
    cpu_info["cpu_info"] = info["brand_raw"] if info.get("brand", None) is None else info["brand"]
    cpu_info["mhz_per_cpu"] = (info["hz_advertised_raw"] if info.get("hz_advertised", None) else info["hz_advertised"])[0] / 1.0e6
